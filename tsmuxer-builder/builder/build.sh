...
cp -v /usr/bin/tsMuxerGUI "$INSTALL_DIR/bin/"
...
for BIN in "$INSTALL_DIR/bin/tsMuxeR" "$INSTALL_DIR/bin/tsMuxerGUI"
do
    DEPS="$(LD_TRACE_LOADED_OBJECTS=1 "$BIN" | grep " => " | cut -d'>' -f2 | sed 's/^[[:space:]]*//' | cut -d'(' -f1 | grep '^/usr\|^/lib')"
...
#!/bin/sh
exec env LD_LIBRARY_PATH=/opt/tsmuxer/lib /opt/tsmuxer/bin/tsMuxeR "$@"
EOF
chmod +x "$INSTALL_DIR/tsMuxeR"
cat << 'EOF' > "$INSTALL_DIR/tsMuxerGUI"
#!/bin/sh
exec env LD_LIBRARY_PATH=/opt/tsmuxer/lib /opt/tsmuxer/bin/tsMuxerGUI "$@"
EOF
chmod +x "$INSTALL_DIR/tsMuxerGUI"

echo "Creating tarball..."
tar -zcf "$TARBALL_DIR/tsmuxer.tar.gz" -C "$INSTALL_BASEDIR" "${ROOT_EXEC_DIR:1}" "lib" --owner=0 --group=0

echo "$TARBALL_DIR/tsmuxer.tar.gz created successfully!"
